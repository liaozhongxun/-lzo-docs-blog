        ...
        -   数值类型
            ...
            -   整型
                ...
                -   三种还区分有符号(默认)和无符号，同样占 32 位的有符号整型，最高位代表 0 正 1 负
                    - 范围不同，表示的大小是一样的，-128和+127间个数 等于0到255的个数
    ...
    - printf
        - int printf(const char *format,...)
            - format:"%[可选修饰符]格式字符，替换逗号后面的变量"
                ...
                - 修饰符
                    - m|.n
                        - 整数j:设置输出宽带，比实际小忽略，否则前面补空格，printf("i = %4d",i)
                        - 浮点数:%8,1f,指定输出需要8位，小数点保留一位四舍五入
                        - 字符串:%.5s，只输出5个字符
                    - %-10d 补空格后左对齐
                    - %010d 补0后左对齐,%后面跟0
                    - %#x:十六进制和八进制输出添加0x或0等前缀
                    - %lxxx:d o x u格式字符，指定输出精度为long，e f g输出精度指定为double 
    - scanf
        - int scanf(const char *format,&变量地址)
            - 修饰符与printf类型，不能有提示信息，并且后面的是变量地址
            - scanf("%d,%f",&i,&f) :动态输入
