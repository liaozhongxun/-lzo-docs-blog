...
-   String 方法提供的方法
    ...
    -   `str.replace(reg，callback|char)`:把匹配到的子字符串进行替换
        ...
        - 将匹配到的数据处理后使用:`"hello".replace(/^\S/, s => s.toUpperCase())`
...
> 正则表达式是由所有字符以及特殊字符(元字符组成)的`文字模式`， 正则表达式作为一个模板，将`文字模式`与搜索到的字符串进行匹配
...
-   字符簇
    -   [a-zA-Z0-9]
        -   [ASCII]从上到下的顺序都可以如：[Z-a]可以匹配 Z、、\[、\],^ 等字符
    -   [abc]: 匹配A或B或C
    -   [^abc]: 取反，就不匹配 A、B、C
    -   [\u4e00-\u9fa5]: 匹配中文
-   `\d`:匹配纯数字，`\D`取反
...
> linux 指令中一般操作文件路径的用`通配符`,如:find，ls，cp 等,操作文件内容的用`正则`,如 grep
